...
namespace F1Solutions.InfrastructureStatistics.ApiCalls.ApiTask
{
    public abstract class BaseApiTask
    {
        ...
        protected async Task<string> GetAllAsync(string uri, HttpMethod method, int attempt = 1, int maxAttempts = 5)
        {
            return await GetAllAsync(uri, Id, Token, method, string.Empty, attempt, maxAttempts);
        }

        protected async Task<string> GetAllAsync(string uri, string id, string token, HttpMethod method, string requestBody = "", int attempt = 1, int maxAttempts = 5)
        {
